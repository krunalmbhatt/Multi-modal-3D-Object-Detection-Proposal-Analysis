...
class MEFormerHead(BaseModule):
    ...
    def forward_single(self, x, x_img, img_metas):
        ...
        print_sample_idx = 0 # Debug prints for this sample index (corresponds to batch item 0 here)
        debug_query_idx = 0 # Debug prints for this query index (in its task's query block)
        debug_dec_idx = 0   # Debug prints for this decoder layer index
        debug_task_id = 0   # Debug prints for this task index (0: car, etc.)
        # Print shapes once per forward pass (first frame) and PC Range
        # Check if it's the first frame (sample_idx 0) and we are in the first task/layer/query for minimal output
        if img_metas[0]['sample_idx'] == 0 and debug_task_id == 0 and debug_dec_idx == 0 and debug_query_idx == 0:
             print(f"\n--- Debug Shapes & PC Range Frame {img_metas[0]['sample_idx']} ---")
             print(f"  PC Range: {self.pc_range}")
             print(f"  x shape: {x.shape}")
             print(f"  x_img shape: {x_img.shape if x_img is not None else 'None'}")
             print(f"  reference_points (initial) shape: {reference_points.shape}") # [num_query, 3]
             print(f"  reference_after_invsigmoid shape: {reference_after_invsigmoid.shape}") # [bs, total_num_queries, 3]
             print(f"  outs_dec_cat shape: {outs_dec_cat.shape}") # [num_dec, bs, total_num_queries, hidden_dim]
             print("------------------------------------")
        # --- End Debug Print Configuration ---
        ...
        for task_id, task in enumerate(self.task_heads, 0):
            ...
            # --- Debug Prints AFTER Sigmoid, BEFORE Denorm ---
            # Check if debug indices are valid based on the shape of 'center' and batch item 0
            # The shape of 'center' after line 260 should be [num_dec, bs, total_num_queries, 2] if broadcasting worked as intended
            # Let's use the shape of 'center' for indexing validity check
            if task_id == debug_task_id and debug_query_idx < outs['center'].shape[2]: # Check if debug_query_idx is valid for this task based on the FINAL shape
                # Loop over decoder layers to print for the debug case
                for dec_id in range(center.shape[0]): # Use the shape from 'center'

                    if dec_id == debug_dec_idx: # Only print for the debug layer

                        print(f"\n--- DEBUG AFTER SIGMOID/BEFORE DENORM Frame {print_sample_idx} - Task {task_id} ({self.class_names[task_id][0]}) - Dec Layer {dec_id} - Query {debug_query_idx} ---")
                        # Print PC Range again for clarity in this block
                        print(f"  PC Range: {self.pc_range}")

                        # Calculate the value input to denormalization (after sigmoid)
                        # Access 'center' and 'height' directly, which are the results of the sigmoid calculation
                        val_input_to_denorm_center = center[dec_id][0, debug_query_idx, :].detach().cpu().numpy() # [2] - Should be in [0, 1] range
                        val_input_to_denorm_height = height[dec_id][0, debug_query_idx, :].detach().cpu().numpy() # [1] - Should be in [0, 1] range

                        print(f"  Value AFTER Sigmoid (Input to Denorm Center): {val_input_to_denorm_center}") # Should be in [0, 1] range
                        print(f"  Value AFTER Sigmoid (Input to Denorm Height): {val_input_to_denorm_height}") # Should be in [0, 1] range

                        # Note: To get values BEFORE sigmoid (raw offset + ref), we would need
                        # to correctly identify the corresponding reference point slice and raw offset
                        # and replicate the sum, which is complex due to the indexing uncertainty.
                        # We'll focus on the values entering and exiting the denormalization for now.


                        print("--------------------------------------------------")
            # --- End Debug Prints AFTER Sigmoid ---
            ...
            outs['center'] = _center
            outs['height'] = _height
            # --- Debug Prints AFTER the potentially problematic calculation and overwrite ---
            # Check if debug indices are valid for this task and batch item 0 based on the FINAL shape of outs['center']
            # The final shape should be [num_dec, bs, task_q, dim] IF the overwrite worked without error (implying task_q == max(task_q, total_q))
            # Let's use the shape of outs['center'] AFTER the overwrite for indexing validity check
            
            if task_id == debug_task_id and debug_query_idx < center.shape[2]: # Check if debug_query_idx is valid for the calculated 'center' shape
                # Loop over decoder layers to print for the debug case
                for dec_id in range(outs['center'].shape[0]): # Use the final shape from outs

                    if dec_id == debug_dec_idx: # Only print for the debug layer

                        print(f"\n--- DEBUG AFTER CALC/DENORM Frame {print_sample_idx} - Task {task_id} ({self.class_names[task_id][0]}) - Dec Layer {dec_id} - Query {debug_query_idx} ---")
                        print(f"  PC Range: {self.pc_range}")

                        # Access the values AFTER they have been overwritten
                        stored_center = outs['center'][dec_id][0, debug_query_idx, :].detach().cpu().numpy() # [2] - This is the physical center (x, y)
                        stored_height = outs['height'][dec_id][0, debug_query_idx, :].detach().cpu().numpy() # [1] - This is the physical z (height)

                        print(f"  Stored Denormalized Center (outs['center']): {stored_center}") # [2] - This is the physical center (x, y)
                        print(f"  Stored Denormalized Height (outs['height']): {stored_height}") # [1] - This is the physical z (height)

                        # Print the full 3D point (center_x, center_y, center_z)
                        print(f"  Stored Denormalized 3D Center: {np.concatenate([stored_center, stored_height])}") # [3]

                        print("--------------------------------------------------")

            # --- End Debug Prints AFTER ---
