...
log_config = dict(
    ...
    hooks=[dict(type='TextLoggerHook'),
           dict(type='TensorboardLoggerHook'),
                   # --- Add this dictionary for Wandb ---
           dict(
                type='WandbLoggerHook',
                init_kwargs=dict(
                    project='MEFormer_Project', # CHANGE THIS to your preferred project name on W&B
                    # entity=None, # Set if using a team/specific user
                    # name=None # Set for a specific run name, otherwise auto-generated
                ),
                interval=50, # How often to log metrics to W&B (syncs with console interval)
                log_artifact=True # Set to True to upload model checkpoints as W&B artifacts (uses storage)
            )
        ])
