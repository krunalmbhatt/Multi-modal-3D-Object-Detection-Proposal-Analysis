...
class MEFormerHead(BaseModule):
    ...
    #     # ret = multi_apply(self.forward_single, pts_feats, img_feats, img_metas)

    #     # batch_proposals = [r for r in ret[0]]  # take first result
    #     return multi_apply(self.forward_single, pts_feats, img_feats, img_metas)
    #     # return {'batch_proposals': batch_proposals}
    def forward(self, pts_feats, img_feats=None, img_metas=None):
        """
        list([bs, c, h, w])
        """
        img_metas = [img_metas for _ in range(len(pts_feats))]
        outs = multi_apply(self.forward_single, pts_feats, img_feats, img_metas)
        return outs
